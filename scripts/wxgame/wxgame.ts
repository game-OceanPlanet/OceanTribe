    ...
    async onFile(file: plugins.File) {
        if (file.extname == '.js') {
            ...
            if (filename == 'libs/modules/egret/egret.js' || filename == 'libs/modules/egret/egret.min.js') {
                ...
                if (filename == "libs/bin/jszip/jszip.js" || filename == 'libs/bin/jszip/jszip.min.js') {
                    content = "!function(t){window.JSZip=t()}" + content;
                    content = content.replace('doc.createElementNS("http://www.w3.org/1999/xhtml",','doc.createElement(')
                    content = content.replace('doc.createElementNS("http://www.w3.org/1999/xhtml",','doc.createElement(')
                }
                ...
                if (filename == 'main.js') {
                    content = "var qmr = window.qmr;" + content;
                }
                if (filename == 'lib-login/login.js' || filename == 'lib-login/login.min.js') {
                    content += "\nwindow.Main = Main;window.qmr = qmr;";
                }
                file.contents = new Buffer(content);
            }
        }
