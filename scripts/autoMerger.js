...
var tempindex = 0;
console.log("start merger textureMerger.....");
//创建输出文件夹
...
var _loop_1 = function (resJson) {
	...
	if (fs.existsSync(resJson) && resJson.indexOf("res.json") > -1) {
		...
		for (var _a = 0, groups_1 = groups; _a < groups_1.length; _a++) {
			...
			// projectName
			tmproject["projectName"] = group.name; 
			// 版本
			tmproject["version"] = 5;
			tempindex++;
			...
			oldkeys = [];
			// files路径
			// console.log("merger ..1" + group.name);
			var urls = newKeys.map(function (key) {
				// console.log("resourcesHash_1:" + key,resourcesHash_1[key]);
				return path.join(pathNor, resourcesHash_1[key]);
			});
			// console.log("merger ..2" + group.name);
...
for (var _a = 0, resjsons_1 = resjsons; _a < resjsons_1.length; _a++) {
	...
	console.log("merger textureMerger....." + resJson);
	_loop_1(resJson);
}
console.log("Complete merger textureMerger.");