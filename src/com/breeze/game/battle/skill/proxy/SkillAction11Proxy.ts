...
{
	/**
	 * @description (11)远程，从受击目标对面垂直飞行过来攻击
	 */
    export class SkillAction11Proxy extends BaseSkillProxy
    {
        ...
        public static getSkillProxy(): SkillAction11Proxy
        {
            var proxy: SkillAction11Proxy = Pool.getItemByClass("SkillAction11Proxy", SkillAction11Proxy);
            return proxy;
        }

        public static recovrySkillProxy(proxy: SkillAction11Proxy): void
        {
            Pool.recover("SkillAction11Proxy", proxy);
        }
            ...
            if (this.skillEffectCfg.show_effect > 0)
            {
                ...
                if (this.targetList && this.targetList.length > 0)
                {
                    ...
                    if (targetActor)
                    {
                        ...
                        if (effect)
                        {
                            let starPos;// = BattlePosionManager.instance.getSkill11PosByType(this.owner.camp, this.owner.pos, true);
                            let endPos;// = BattlePosionManager.instance.getSkill11PosByType(targetActor.camp, targetActor.pos);
                            starPos.x = endPos.x;
                            var dx = 0;
        ...
        public recycleSkill(): void
        {
            ...
            SkillAction11Proxy.recovrySkillProxy(this);
        }
    }
}