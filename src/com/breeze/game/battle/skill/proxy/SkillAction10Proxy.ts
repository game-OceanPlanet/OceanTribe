module qmr
{
	 ...
	 */
    export class SkillAction10Proxy extends BaseSkillProxy
    {
        ...
        public static getSkillProxy(): SkillAction10Proxy
        {
            var proxy: SkillAction10Proxy = Pool.getItemByClass("SkillAction10Proxy", SkillAction10Proxy);
            return proxy;
        }

        public static recovrySkillProxy(proxy: SkillAction10Proxy): void
        {
            Pool.recover("SkillAction10Proxy", proxy);
        }
        ...
        protected onReleaseSkillEffect(): void
        {
            ...
            if (this.skillEffectCfg.show_effect > 0)
            {
                ...
                if (this.targetList.length > 0)
                {
                    ...
                    if (targetActor)
                    {
                        var effect: BaseEffect = EffectPlayer.playEffect(this.skillEffectCfg.show_effect, this.owner, -1, this.owner.timeScale);
                        if (effect)
                        {
        ...
        {
            var effect: BaseEffect = EffectPlayer.playEffect(this.skillEffectCfg.orderly_effect, this._centerPos, -1, this.owner.timeScale);
            this.addTempEffect(effect);
